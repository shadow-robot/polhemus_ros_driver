...
class SrGloveCalibration():
    def __init__(self):
        ...
        self._index = rospy.get_param("~base_index")
    ...
    '''
    alternative method 'Circle Hough Transform'
    def _sphere_fit(self, data):
        ...
        for i in range(len(x)):
            for r in np.arange(0.075,0.13,0.01):
                for a in np.arange(0, 2*math.pi, 0.1):
                    for b in np.arange(0, math.pi, 0.1):
                        cx = round(x[i] - (r * math.cos(a) * math.sin(b)), precision)
                        cy = round(y[i] - (r * math.sin(a) * math.sin(b)), precision)
                        cz = round(z[i] - (r * math.cos(b)), precision)
                        r = round(r, precision)
                        tupe = (cx, cy, cz, r)
                        if tupe in accumulator_r:
                            accumulator_r[tupe] += 1
                        else:
                            accumulator_r.update({tupe: 0})
        print(max(accumulator_r, key=accumulator_r.get))
        return max(accumulator_r, key=accumulator_r.get)
    '''
