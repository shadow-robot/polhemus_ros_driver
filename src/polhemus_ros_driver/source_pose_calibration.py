...
class SourcePoseCalibration(object):
    ...
    def check_if_values_within_range(self, knuckle_thickness, knuckle_to_source):
        if not knuckle_thickness_accepted_range[0] <= knuckle_thickness <= knuckle_thickness_accepted_range[1]:
            raise ValueError("Knuckle thickness outside of accepted range")
        if not knuckle_to_source_accepted_range[0] <= knuckle_to source <= knuckle_to_source_accepted_range[1]:
