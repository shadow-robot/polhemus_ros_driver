...
class SourcePoseCalibration(object):
    def __init__(self):
        ...
        self.knuckle_thickness_accepted_range = range(0.015, 0.045)
        self.knuckle_to_source_accepted_range = range(0.01, 0.06)
    ...
    def check_if_values_within_range(self, knuckle_thickness, knuckle_to_source):
        if not knuckle_thickness in self.knuckle_thickness_accepted_range:
            raise ValueError("Knuckle thickness outside of accepted range")
        if not knuckle_to_source in self.knuckle_to_source_accepted_range:
